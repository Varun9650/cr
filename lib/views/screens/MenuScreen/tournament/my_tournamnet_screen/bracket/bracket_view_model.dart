...
class BracketViewModel extends ChangeNotifier {
  ...
  Future<bool> saveCurrentBracket(
    ...
    try {
      ...
      if (_currentBracket == null) {
        throw Exception('No bracket to save');
      }
  ...
  void shuffleRoundTeams(int round) {
    _bracketService.shuffleRoundTeams(_bracketStructure, round);
    notifyListeners();
  }
...
// before point tables
  ...
  // Helper methods
  int _generateTeamId(String teamName) {
    // This is a simple hash function - in real app, you'd get actual team IDs
...
// Before Point Tables
  ...
  List<String> getAvailableTeamsForRound(int round) {
    final matchesForRound =
        _bracketStructure.where((m) => m['round'] == round).toList();
    List<String> teams = [];
    for (var match in matchesForRound) {
      if (match['team1'] != null && match['team1'] != 'BYE')
  ...
  Future<int> assignTeamsToRound(int round, List<String> teamNames) async {
