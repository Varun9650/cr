...
class TeamViewModel extends ChangeNotifier {
  ...
  void selectTeam(int index, bool isEnrolled) {
    if (isEnrolled) {
      ...
      if (_enrolledTeams.isNotEmpty && index < _enrolledTeams.length) {
        final teamId = int.tryParse(_enrolledTeams[index].id);
        if (teamId != null && teamId > 0) {
          fetchTeamMembers(teamId);
        } else {
          print(
              'Invalid team ID for enrolled team: ${_enrolledTeams[index].id}');
          _teamMembers = [];
          notifyListeners();
        }
      }
